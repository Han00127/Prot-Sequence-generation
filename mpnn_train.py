...
def main(args):
    ...
    for e in range(args.epoch):
        ...
        t1 = time.time()
        print("Times for one epoch ", t1 - t0)
        ...
        writer.add_scalar('Short perplexity', short_test_perplexity, e)
        writer.add_scalar('Short median recovery', short_recovery, e)
        writer.add_scalar('Single perplexity', sc_test_perplexity, e)
        ...
        checkpoint_filename = base_folder+'model_weights/last{}.pt'.format(e+1)
        torch.save({
                'epoch': e+1,
                'step': total_step,
...
if __name__ == "__main__":
    ...
    main(args)
        ...
        #         short_test_sum += torch.sum(loss * mask_for_loss).cpu().data.numpy()
        #         short_test_weights += torch.sum(mask_for_loss).cpu().data.numpy()
        #         short_test_recovery.append(calculate_recovery(log_probs,S))
        ...
        #         sc_test_sum += torch.sum(loss * mask_for_loss).cpu().data.numpy()
        #         sc_test_weights += torch.sum(mask_for_loss).cpu().data.numpy()
        #         sc_test_recovery.append(calculate_recovery(log_probs,S))


    #         ## Loads CATH 4.2 Short test dataset
    # file_splits='/data/project/rw/cath4.2/test_split_L100.json'
    # with open(file_splits) as f:
    #     dataset_splits = json.load(f)
    # short_test_set = Subset(dataset, [dataset_indices[chain_name] for chain_name in dataset_splits['test'] if chain_name in dataset_indices])
